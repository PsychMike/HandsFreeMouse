...
function [pupil_x,disL,disR] = grab_eye_cords(snapshot)
...
%     vid=getsnapshot(cam);  %get a snapshot of webcam

    snapshot = rgb2gray(snapshot);    %convert to grayscale
    img = flip(snapshot, 2); % Flips the image horizontally
